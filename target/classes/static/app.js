...
// Logout - CORREGIDO para limpiar todo
async function logout() {
    try {
        ...
        // Limpiar tablas
        document.querySelector('#notasTable tbody').innerHTML = '';
        document.querySelector('#materiasTable tbody').innerHTML = '';
        document.querySelector('#usuariosTable tbody').innerHTML = '';
        
        // Limpiar filtros
        document.getElementById('filterMateria').innerHTML = '<option value="">Todas las materias</option>';
        document.getElementById('filterEstudiante').innerHTML = '<option value="">Todos los estudiantes</option>';
        document.getElementById('filterRol').value = '';
        
        // Resetear a tab de notas
        document.querySelectorAll('.tab').forEach(tab => tab.classList.remove('active'));
        document.querySelectorAll('.tab-content').forEach(content => content.classList.remove('active'));
        document.querySelector('.tab').classList.add('active');
        document.getElementById('notasTab').classList.add('active');
...
function showMainScreen() {
    ...
    if (currentUser.rol === 'Alumno') {
        ...
        document.getElementById('filterEstudianteGroup').classList.add('hidden');
        const actionsColumns = document.querySelectorAll('.actions-column');
        actionsColumns.forEach(col => col.classList.add('hidden'));
    } else if (currentUser.rol === 'Profesor') {
        document.getElementById('tabPromedio').classList.add('hidden');
        document.getElementById('filterEstudianteGroup').classList.remove('hidden');
    }
...
function populateEstudianteFilter() {
...
// Aplicar filtros de notas
function applyNotasFilter() {
    const materiaId = document.getElementById('filterMateria').value;
    const estudianteId = document.getElementById('filterEstudiante').value;
    
    let notasFiltradas = [...notasOriginales];
    
    if (materiaId) {
        notasFiltradas = notasFiltradas.filter(n => n.materia.id == materiaId);
    }
    
    if (estudianteId) {
        notasFiltradas = notasFiltradas.filter(n => n.estudiante.id == estudianteId);
    }
...
// Limpiar filtros de notas
function clearNotasFilter() {
...
function applyUsuariosFilter() {
    ...
    if (rol) {
        usuarios = usuariosOriginales.filter(u => u.rol === rol);
    } else {
...
function renderNotas() {
    ...
    if (notas.length === 0) {
        tbody.innerHTML = '<tr><td colspan="8" style="text-align: center; padding: 20px; color: #999;">No hay notas para mostrar</td></tr>';
        return;
    }
...
function renderUsuarios() {
    ...
    if (usuarios.length === 0) {
        tbody.innerHTML = '<tr><td colspan="6" style="text-align: center; padding: 20px; color: #999;">No hay usuarios para mostrar</td></tr>';
        return;
    }
...
async function loadPromedio() {
    try {
        ...
        const materiaIds = [...new Set(notasOriginales.map(n => n.materia.id))];
        let materiasHtml = '<h3 style="margin-top: 30px;">Notas por Materia</h3>';
        
        for (const materiaId of materiaIds) {
            const notasMateria = notasOriginales.filter(n => n.materia.id === materiaId);
...
async function editNota(id) {
    const nota = notasOriginales.find(n => n.id === id);
    if (!nota) return;
...
function populateEstudianteSelect() {
    ...
    usuariosOriginales.filter(u => u.rol === 'Alumno').forEach(usuario => {
        select.innerHTML += `<option value="${usuario.id}">${usuario.nombre}</option>`;
    });
}
...
function setupForms() {
    // Ya configurados con addEventListener
}
